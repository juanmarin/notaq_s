...
printer_draw_text($handle,"::       REPORTE CLIENTES FALTANTES       ::",0,$li);
...
while($r = $db->fetchNextObject($res)) {
	...
	$li+=$alto*2;	printer_draw_text($handle,'Fecha: ' . moneda($r->fecha, 0), 0,$li);
	$li+=$alto;		printer_draw_text($handle,'Nombre: ' . $r->nombre . ' ' . $r->apellidop . ' ' . $r->apellidom, 0,$li);
	$li+=$alto;		printer_draw_text($handle,'Monto: $' . moneda($pago, 0), 0,$li);
