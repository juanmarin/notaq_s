...
switch($_POST["action"]){
	...
	case "recargos_condonar":
		$sql="SELECT nivel FROM mymvcdb_users WHERE usuario = '".$_POST["usuario"]."' AND password = '".sha1($_POST["autorizacion"])."'";
		$res=mysql_query($sql);
		$au = mysql_fetch_array($res);
		if((mysql_num_rows($res) > 0) && ($au[0] == 0)){
			$sql="UPDATE recargos SET estado = 2 WHERE estado=0 AND cuenta=".$_POST["c"];
			mysql_query($sql);
		}
		echo '<meta http-equiv="refresh" content="0;url=../../?pg=2e&cl='.$_POST["cl"].'"> ';
		break;
	case "recargos_pagos":
		...
		if((mysql_num_rows($res) > 0) && ($au[0] == 0)){
			...
			$sql="SELECT tipo_pago FROM cuenta WHERE id=".$_POST["c"];
			$res=mysql_query($sql);
			$c=mysql_fetch_array($res);
			$tipoPago=$c[0];
			...
			$sql="INSERT INTO pagos (cliente, cuenta, fecha, pago, interes)VALUES(".$_POST["cl"].", ".$_POST["c"].", '".$fecha."', ".$recargos.", 0)";
			mysql_query($sql);
