...
## SACANDO LOS DIAS DE PAGOS SEGUN EL TIPO Y DIAS ##
if($_POST["tipo_pago"] < 4){
		$diaPago =  getDiaSemana2($_POST["dias_pago"], $_POST["tipo_pago"]);
	}else{
...
$li+=$alto;
printer_draw_text($handle,"Fecha: ".$c["fecha"],0,$li);
$li+=$alto;
...
$li+=$alto;
printer_draw_text($handle,"Plazo: ".$c["tiempo"]." MESES",0,$li);
$li+=$alto;
printer_draw_text($handle,"Total con interes: ".moneda2($c["total"]),0,$li);
$li+=$alto;
printer_draw_text($handle,$c["npagos"]." pagos de $".getPagoRedondo($c["pago"]),0,$li);
$li+=$alto;
printer_draw_text($handle,"Tipo de pago: ".$tp,0,$li);
$li+=$alto;
printer_draw_text($handle,"Dias de pago: ".$diaPago,0,$li);
$li+=$alto;
