...
<table>
	...
	while($cob = $db->fetchNextObject($res))
	{
		...
		echo "<td>";
			...
			$ancho = ( $azul / $suma ) * 100;
			echo "<div style='background-color:#000080;height:11px;width:$ancho%;float:left;' title='$azul clientes, ".number_format(2,2,".",$ancho)." %'></div>";
			$ancho = ( $verde / $suma ) * 100;
			echo "<div style='background-color:#009900;height:11px;width:$ancho%;float:left;' title='$verde clientes, ".number_format(2,2,".",$ancho)." %'></div>";
			$ancho = ( $amarillo / $suma ) * 100;
			echo "<div style='background-color:#FFFF00;height:11px;width:$ancho%;float:left;' title='$amarillo clientes, ".number_format(2,2,".",$ancho)." %'></div>";
			$ancho = ( $rojo / $suma ) * 100;
			echo "<div style='background-color:#FF0000;height:11px;width:$ancho%;float:left;' title='$rojo clientes, ".number_format(2,2,".",$ancho)." %'></div>";
			$ancho = ( $negro / $suma ) * 100;
			echo "<div style='background-color:#000000;height:11px;width:$ancho%;float:left;' title='$negro clientes, ".number_format(2,2,".",$ancho)." %'></div>";
