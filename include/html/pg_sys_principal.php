...
{
	$hoy = date("Y-m-d");
	$nxt = date('Y-m-d', strtotime('+7 days'));
	$sql = "SELECT username FROM mymvcdb_users WHERE nivel = 3";
	$res = $db->query($sql);
	while($c=$db->fetchNextObject($res))
	{
		$cob = $c->username;
		$sql = "SELECT count(*) cobrospc
		FROM cuentas cu, pagos pa
		WHERE cu.id=pa.cuenta AND cu.estado=0 AND pa.estado=0 AND pa.fecha BETWEEN CAST('$hoy' AS DATE) AND CAST('$nxt' AS DATE)
		AND cu.cobrador='$cob'";
		//echo "$sql<br />";
		$re2 = $db->query($sql);
		$get = $db->fetchNextObject($re2);
		$xco = $get->cobrospc;
		$sql = "INSERT INTO desempeno (semana,cobrador,por_cobrar)VALUES(".date("W").", '".$c->username."',$xco)";
		$db->execute($sql);
	}
}
///////////////////////////////////////////////////////////////////////////////////////////////////
